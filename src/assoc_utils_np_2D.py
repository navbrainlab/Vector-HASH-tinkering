...
def gen_gbook_2d(lambdas, Ng, Npos):
    ...
    for x in range(Npos):
        # Traverse all spatial positions
        for y in range(Npos):
            index = 0
            for period in lambdas:
                phi1, phi2 = x % period, y % period 
                # this is For a module with a period of λ, it will form a λ x λ periodic grid in space.

                gpattern = np.zeros((period, period))
                gpattern[phi1, phi2] = 1
                # In the cyclic pattern, highlight the position within the current cycle (equivalent to the only activated unit in the current grid cell module).

                gpattern = gpattern.flatten()
                # Flatten the 2D pattern to a 1D vector
